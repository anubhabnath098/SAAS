...
export async function GET(req: NextRequest) {
  ...
  try {
    if(!productId){
        const chartData = await getViewsByCountryChartData({ timezone, userId, interval });
        return NextResponse.json(chartData, { status: 200 });
    }else{
        const chartData = await getViewsByCountryChartData({ timezone, userId, interval, productId });
        return NextResponse.json(chartData, { status: 200 });
    }
