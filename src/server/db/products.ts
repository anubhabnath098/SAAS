...
export async function updateCountryDiscounts(
    deleteGroup:{ countryGroupId: string}[]| undefined,
    ...
    if(deleteGroup != null && deleteGroup.length>0){
        statements.push(db.delete(CountryGroupDiscountTable).where(
                and(eq(CountryGroupDiscountTable.productId, productId),
    ...
    if(insertGroup.length>0){
        statements.push(db.insert(CountryGroupDiscountTable).values(insertGroup)
    .onConflictDoUpdate({
        target:[
...
async function getProductForBannerInternal({
    ...
    const basicCheck = await db.query.ProductTable.findFirst({
    ...
    console.log(discount, country, product)
