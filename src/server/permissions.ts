...
export async function canCustomizeBanner(userId:string|null|undefined){
...
export async function canCreateProduct(userId: string | null | undefined){
    if(userId == null || userId==undefined) return false;
    const tier = await getUserSubscriptionTier(userId);
...
export async function canShowDiscountBanner(userId: string | null | undefined){
    ...
    return tier.maxNumberOfVisits> productViews
}