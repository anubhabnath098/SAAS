...
//refreshes every component in the website: ex-->Changing a country discount
export function getGlobalTag(tag: keyof typeof CACHE_TAGS){
    return `global: ${CACHE_TAGS[tag]}` as const
}

//user level refreshing: ex--> user changes username or deletes username
export function getUserTag(userId:string, tag: keyof typeof CACHE_TAGS){
    return `user:${userId}-${CACHE_TAGS[tag]}` as const
}

//individual cache: ex--> change in a particular product like editing or deleting
